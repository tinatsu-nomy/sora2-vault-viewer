...
function createSerializers({ debugMode, enableSqliteCache, runtimePaths = {} }) {
  ...
  function serializeListItem(item) {
    return {
      ...
      cameoCount: Number(item.cameoCount || 0),
  ...
  function serializeListRow(row) {
    return {
      ...
      kind: row.kind,
      source: row.source,
      sourceMemberships: parseJson(row.sourceMembershipsJson, row.source ? [row.source] : []),
      date: row.date,
      prompt: row.prompt,
      genId: row.genId,
      ...
      cameoCount: Number(row.cameoCount || 0),
  ...
  function serializeDetailItem(item) {
    return {
      ...
      posterDisplayName: item.posterDisplayName || null,
      posterDescription: item.posterDescription || null,
      cameoCount: Number(item.cameoCount || 0),
      cameoOwnerUsernames: item.cameoOwnerUsernames || [],
